...
async def main(_, msg: Message):
    status = await msg.reply("**- انني افعل فعل أفضل ما أفعله...**")
...
async def process(msg: Message, stark: Helpers, status: Message):
    ...
    if os.path.exists(stark.output_file):
        ...
        await status.edit('**- جـارِ جـلب حزمـة الملصقـات...**')
        ...
        await status.delete()
        if sticker:
            button = [[InlineKeyboardButton('◟𓄂 𝙕𝙚𝙙𝙏𝙝𝙤𝙣◞', callback_data='current_webm')]]
...
async def existing_sticker_func(_, msg: Message):
    if msg.sticker.is_video:
        ...
        if data['kang_mode']:
            status = await msg.reply("**- انني افعل فعل أفضل ما أفعله [جـارِ]...**")
