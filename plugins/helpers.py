...
class Helpers(BotAPI):
    ...
    async def user_settings(self) -> tuple[bool, bool] | tuple[str, InlineKeyboardMarkup]:
        ...
        if ask_emojis_db:
            ask_emojis += tick
            text += f"**سؤال الايموجي** : True"
        else:
            ask_emojis += cross
            text += f"**سؤال الايموجي** : False"
    ...
    async def ask_for_emojis(self):
        emojis_msg = await self.client.ask(
            self.user_id,
            "**- الرجاء إرسال الايموجي لي لإضافته لـ هذا الملصق.**",
            filters=filters.text & filters.incoming
        )
        status = await emojis_msg.reply('**- جاري فحص الملف...**')
        emojis = await self.extract_emojis(emojis_msg)
        if not emojis:
            await status.edit('ايموجي غير صالح. تم إلغاء العملية.')
            return None
    ...
    async def correct_the_size(self):
        ...
        kib = info // 1024
        if kib > 256:
            await self.status.edit('تغيير حجم الفيديو ليلائم حدود تيليجـرام...')
