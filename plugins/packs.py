...
@Stark.cmd('packs', description="لـ الحصول على قائمة الحزم الخاصة بك")
async def packs_func(_, msg: Message):
    if msg.from_user.id not in OWNER_ID:
        ...
        if not packs:
            await msg.reply('ليس لديك حزم بعد. قم بإنشاء واحدة عن طريق إرسال فيديو أو GIF', quote=True)
            return
        if packs == 1:
            string = '**حزمتـك** \n\n'
        else:
